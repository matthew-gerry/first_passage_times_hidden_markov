...
for i in range(len(Nrange)):
    ...
    L = rm.L_impurity(N, 0, k, 0.8*k, b) # Impurity placed at site 0 so it works for all site lengths
    start_site = 0; leak_site = N - 1 # Choice of start and leak sites
...
fig, ax = plt.subplots()
ax.scatter(Nrange, M3_N_uni, label="Uniform")
for i, N in enumerate(Nrange):
    ax.annotate("N = "+str(N), (M1_N_uni[i], r_uni[i]))
ax.scatter(Nrange, M3_N_mod, label="Modular")
ax.scatter(Nrange, M3_N_imp, label="Impurity")
ax.set_xlabel(r"$N$")
# ax.set_ylabel(r"$\langle t^2\rangle$")
ax.set_ylabel(r"$\langle t^3\rangle$")
plt.legend()
plt.show()